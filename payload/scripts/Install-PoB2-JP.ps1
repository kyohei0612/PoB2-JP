...
"`t`tfunction DrawString(x, y, align, height, font, text)",
"`t`t`tif type(text) == `"string`" then",
"`t`t`t`tlocal _tr = poejpSafeTranslate(text)",
"`t`t`t`tif _tr ~= text and type(height) == `"number`" and #text <= 30 then",
"`t`t`t`t`tlocal _ew = _DrawStringWidth(height, font, text)",
"`t`t`t`t`tlocal _jw = _DrawStringWidth(height, font, _tr)",
"`t`t`t`t`tif _ew > 0 and _jw > _ew * 1.1 then",
"`t`t`t`t`t`tlocal _h = height * _ew / _jw",
"`t`t`t`t`t`tlocal _min = height * 0.6",
"`t`t`t`t`t`tif _h < _min then _h = _min end",
"`t`t`t`t`t`theight = _h",
"`t`t`t`t`tend",
"`t`t`t`tend",
"`t`t`t`ttext = _tr",
"`t`t`tend",
